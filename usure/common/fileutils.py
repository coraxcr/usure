...
def read_files(folderpath, extensions, encoding) -> Iterator[Tuple[str, Callable[[],Iterator[str]]]]:
    ...
    filenames = [dirname for dirname in dirnames if path.isfile(path.join(folderpath, dirname))]
    filenames = [filename for filename in filenames if Path(filename).suffix in extensions]
    filenames.sort(key = lambda filename: path.getsize(path.join(folderpath, filename)))
