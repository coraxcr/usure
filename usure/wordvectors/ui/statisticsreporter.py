...
class StatisticsReporter:
    ...
    def plot_scatter(self, numb_words=1000, num_dim=2):
        vocabulary, vectors = self._get_vocabulary_and_vectors(self._keyed_vectors, numb_words)
    ...
    def _order_by_frequence(self, vocab:Dict[str, int]):
        ...
        sorted_vocabulary = sorted(([k,v] for k,v in vocab_items), key=lambda item: item[1], reverse=True)
        return sorted_vocabulary

    def _get_vocabulary_and_vectors(self, keyed_vectors:KeyedVectors, numb_words:int):
        vocabulary = np.array([word for word,vocab in keyed_vectors.vocab.items()][:numb_words], dtype=object)
        vectors = np.array([keyed_vectors.get_vector(word) for word in vocabulary])
